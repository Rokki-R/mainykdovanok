...
function SearchResultsByQueryPage() {
  ...
  const currentDevices = devices
    ? devices.slice(indexOfFirstItem, indexOfLastItem)
    : [];
      ...
      location.state.searchResults.length > 0 ? (
        <Row className="justify-content-center">
          <h3 style={{ textAlign: "center", marginBottom: "50px" }}>
            Rasti prietaisai pagal "{location.state.searchQuery}" raktažodį
          </h3>
        ...
        <h3 style={{ textAlign: "center", marginTop: "50px" }}>
          Nerasta jokių prietaisų pagal "{location.state.searchQuery}" raktažodį
        </h3>
      )}
      {totalPages > 1 && (
        <Pagination className="justify-content-center">
          {[...Array(totalPages)].map((_, index) => (
            <Pagination.Item
              ...
              style={{ backgroundColor: "#0d3c34", borderColor: "#0d3c34" }}
            >
              {index + 1}
            </Pagination.Item>
          ))}
