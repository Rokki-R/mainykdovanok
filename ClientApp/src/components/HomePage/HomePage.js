...
import { Container, Row, Col, Card, Button, Spinner, Pagination, Form, Dropdown } from 'react-bootstrap';
...
function HomePage() {
    ...
    const [selectedCategory, setSelectedCategory] = useState('Visi skelbimai');
    ...
    const filteredDevices = devices ? devices.filter(device =>
        ...
        (selectedCategory === 'Visi skelbimai' || device.category === selectedCategory) &&
        (selectedType === 'Visi tipai' || device.type === selectedType)
    ) : [];
    ...
    const totalPages = devices ? Math.ceil(filteredDevices.length / itemsPerPage) : 0;

    const contentWrapperClass = currentDevices.length > 0 ? 'content-wrapper' : 'content-wrapper-centered';
    ...
    return (
        <Container className="home">
            <Row className="justify-content-center">
                <Col sm={12}>
                    <h3 style={{ textAlign: "center", marginBottom: "25px" }}>Elektronikos prietaisų skelbimai</h3>
                </Col>
            </Row>
            <Row className="justify-content-center">
    <Col sm={6}>
    ...
    <Col sm={3} style={{marginRight: '10px'}}>
        <Dropdown>
            ...
            <Dropdown.Menu>
                <Dropdown.Item onClick={() => handleCategorySelect('Visi skelbimai')}>Visi skelbimai</Dropdown.Item>
                <hr />
                {categories.map(category => (
                    <Dropdown.Item key={category.id} onClick={() => handleCategorySelect(category.name)}>{category.name}</Dropdown.Item>
                ))}
            </Dropdown.Menu>
        </Dropdown>
    </Col>
    <Col sm={3}>
        <Dropdown>
            ...
            <Dropdown.Menu>
                <Dropdown.Item onClick={() => handleTypeSelect('Visi tipai')}>Visi tipai</Dropdown.Item>
                <hr />
                {types.map(type => (
                    <Dropdown.Item key={type.id} onClick={() => handleTypeSelect(type.name)}>{type.name}</Dropdown.Item>
                ))}
            </Dropdown.Menu>
        </Dropdown>
    </Col>
                    ...
                    <div className={contentWrapperClass}>
                        <Row className="justify-content-center">
                            {currentDevices.length > 0 ? currentDevices.map((device) => (
                                ...
                                <Col sm={12}>
                                    {searchTerm || selectedCategory !== 'Visi skelbimai' ? (
                                        <h3>Nerasta jokių skelbimų</h3>
                                    ) : (
